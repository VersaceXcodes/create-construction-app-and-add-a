...
app.get('/api/products', async (req, res) => {
    try {
        ...
        if (category) {
            ...
            const subcatResult = await pool.query(`WITH RECURSIVE category_tree AS (
          SELECT category_id FROM categories WHERE category_id = $1
          UNION
          SELECT c.category_id FROM categories c
          INNER JOIN category_tree ct ON c.parent_category_id = ct.category_id
        )
        SELECT category_id FROM category_tree`, [category]);
        ...
        const mainQuery = `SELECT p.*, s.shop_name, s.rating_average as supplier_rating, s.is_verified as supplier_verified,
                       ...
                       JOIN suppliers s ON p.supplier_id = s.supplier_id
                       WHERE ${whereClause}
                       ORDER BY ${sortMap[sort_by] || sortMap.created_at}
